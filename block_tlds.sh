...
for ip in "${IPV4_TARGETS[@]}"; do
  iptables -A OUTPUT -d "$ip" -p udp --dport 53 -j DROP
  iptables -A OUTPUT -d "$ip" -p tcp --dport 53 -j DROP
done

# Apply IPv6 rules
for ip in "${IPV6_TARGETS[@]}"; do
  ip6tables -A OUTPUT -d "$ip" -p udp --dport 53 -j DROP
  ip6tables -A OUTPUT -d "$ip" -p tcp --dport 53 -j DROP
done
