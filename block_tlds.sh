...
echo "Applying iptables rules to block outbound DNS traffic to specified TLDs..."

# Apply IPv4 rules
for ip in "${IPV4_TARGETS[@]}"; do
  iptables -I DOCKER-USER -d "$ip" -p udp --dport 53 -j DROP
  iptables -I DOCKER-USER -d "$ip" -p tcp --dport 53 -j DROP
done

# Apply IPv6 rules
for ip in "${IPV6_TARGETS[@]}"; do
  ip6tables -I DOCKER-USER -d "$ip" -p udp --dport 53 -j DROP
  ip6tables -I DOCKER-USER -d "$ip" -p tcp --dport 53 -j DROP
done

echo "Done. BIND will now time out when querying these nameservers."
